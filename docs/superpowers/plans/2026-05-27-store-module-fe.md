...
**Goal:** Build the Store module FE (full CRUD + Store Member stub) and the Franchisee FE API layer used for Store filter/form dropdowns. Route constants are added as part of this plan — no prerequisite plan required.
...
- `PagedResult<T>` exists at `@/core/types/pagination.types`
- `useListPage` is at `@/components/ui/composables` (barrel index exports it)
- `useFilters` exposes `setFilters`, `resetFilters`, `activeFilters`
- `DEFAULT_PAGINATION.LIMIT = 10` is exported from `@/core/constants/_index`
- `PagedApiResponse<T>` is at `@/core/models/common.dto`
- `brandService.getBrandsByUserIdAsync(userId)` exists, returns `BrandViewModel[]`
- `useAsyncState` at `@/composables/useAsyncState` returns `{ data, loading, error, execute }`
- User ID is at `useUserStore().profile?.Id` (PascalCase) from `@/modules/user/stores/user.store`
- `BrandClient` constructor pattern uses `super({ baseURL: ENV_URL })`
- `routes.ts` currently has one hardcoded `admin:stores` entry at `path: 'stores'` — Task 23 replaces it
...
| File | Action | Purpose |
|---|---|---|
| `src/core/constants/app-routes.constants.ts` | Modify | Add FRANCHISEES, STORES, STORE_DETAIL, STORE_MEMBERS |
...
| `src/modules/brand/services/franchisee.service.ts` | Create | Returns FranchiseeDto[] for dropdowns |
...
| `src/modules/store/constants/store-member-list.constants.ts` | Create | Columns for member list |
...
| `src/router/routes.ts` | Modify | Replace hardcoded admin:stores entry with real routes |

---

## Task 1: Add route constants

**Files:** Modify `src/core/constants/app-routes.constants.ts`
...
git commit -m "feat: add franchisees, stores, store-detail, store-members route constants"
```

---

## Task 2: Fix `api.constants.ts`
...
## Task 4: Delete stray placeholder
...
- [ ] Run type-check: `npm run type-check` — expected: PASS (nothing imports this file)

- [ ] Commit:
```bash
git rm src/modules/store/composables/useUser.ts
git commit -m "chore: remove stray useUser placeholder from store module"
```

---

## Task 5: Franchisee DTO + API

**Files:**
- Create `src/modules/brand/models/dtos/franchisee.dto.ts`
- Create `src/modules/brand/api/franchisee.api.ts`

- [ ] Create `franchisee.dto.ts` — PascalCase, matches `GetFranchiseeResponse` on BE (no `OwnerUserId`, no `Currency`):
...
## Task 6: Franchisee Service

**Files:** Create `src/modules/brand/services/franchisee.service.ts`

- [ ] Create the file — returns raw DTOs (no ViewModel mapping; used only for dropdown options):
...
- [ ] Create `store.dto.ts` — matches `GetStoreResponse` on BE:
...
- [ ] Create `store-filter.dto.ts` — matches `StoreFilterDto` on BE:
...
- [ ] Create `update-store.dto.ts` — BrandId, FranchiseeId, Code are excluded (BE does not allow updating them):
...
- [ ] Create `store.view-model.ts` — `extends Record<string, unknown>` is required by `AppDataTable`:
...
- [ ] Create `store.model.ts` — all fields including create-only ones; mapper excludes BrandId/FranchiseeId/Code for update requests:
...
- [ ] Create `useStore.ts` — toast-wrapped, mirrors `useBrand.ts` pattern:
...
- [ ] Commit:
```bash
git add src/modules/store/components/StoreList.vue
...
- [ ] Create the file — dialog with 4 sections; `isEdit` prop hides `BrandId`, `FranchiseeId`, `Code` fields:
...
User ID comes from `useUserStore().profile?.Id` (PascalCase `Id`). Import path: `@/modules/user/stores/user.store`.
...
## Task 22: Update `routes.ts`

**Files:** Modify `src/router/routes.ts`

The current file has a hardcoded `admin:stores` entry. Replace it with 4 entries using the new route constants: `FRANCHISEES` (ComingSoon), `STORES` (real view), `STORE_DETAIL` (real view), `STORE_MEMBERS` (real view).

- [ ] Find this exact block in `routes.ts`:
...
git commit -m "feat: register store routes and wire real views (replace ComingSoon admin:stores)"
```
